...
public class CLI {
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if (args.has(OPT_RESIGN)) {
            ...
            X509CertificateHolder holder = file2holder(f);
            X509Certificate crt = new JcaX509CertificateConverter().setProvider(BouncyCastleProvider.PROVIDER_NAME).getCertificate(holder);
            X509Certificate newcert = ca.cloneUserCertificate(crt.getPublicKey(), crt);
            ...
            if (args.has(OPT_PERSO)) {
                ...
                if (args.has(OPT_NEW)) {
                    ...
                    if (args.has(OPT_AUTHCERT)) {
                        EstEIDManager.loadCertificate(sc, file2holder((File) args.valueOf(OPT_AUTHCERT)).getEncoded(), 0);
                    }
                    if (args.has(OPT_SIGNCERT)) {
                        EstEIDManager.loadCertificate(sc, file2holder((File) args.valueOf(OPT_SIGNCERT)).getEncoded(), 1);
                    }
    ...
    static X509CertificateHolder file2holder(File f) throws IOException {
        try (PEMParser pem = new PEMParser(new InputStreamReader(new FileInputStream(f), "UTF-8"))) {
            return (X509CertificateHolder) pem.readObject();
        }
    }
